...
def process_recipes(json_input):
    ...
    with open('recipe.csv', 'w', newline='') as recipe_file, \
         ...
         open('step.csv', 'w', newline='') as step_file:
        ...
        step_writer.writerow(['number', 'description', 'name', 'cuisine_name'])

        # Process each recipe
        for data in recipes:
            ingredient_texts = [ing for ing in data['ingredients']]
            ...
            for index, step in enumerate(data['steps'], start=1):
                step_writer.writerow([index, step, data['name'], data['cuisine_name']])
